...
struct ssd_info *initiation(struct ssd_info *ssd) {
    ...
    //初始化 nvm
    ssd->nvm_head = (struct nvm_info *) malloc(sizeof(struct nvm_info));
    alloc_assert(ssd->nvm_head, "ssd->nvm_head");
    memset(ssd->nvm_head, 0, sizeof(struct nvm_info));
    initialize_nvm(ssd);
...
struct parameter_value *load_parameters(char parameter_file[30]) {
    ...
    while (fgets(buf, 200, fp)) {
        ...
        } else if ((res_eql = strcmp(buf, "nvm capacity")) == 0) {
            sscanf(buf + next_eql, "%d", &p->nvm_capacity);
        } else if ((res_eql = strcmp(buf, "nvmpage capacity")) == 0) {
            sscanf(buf + next_eql, "%d", &p->nvmpage_capacity);
        } else if ((res_eql = strcmp(buf, "nvmpage number")) == 0) {
            sscanf(buf + next_eql, "%d", &p->nvmpage_num);
