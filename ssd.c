...
int main() {
    ...
    ssd = initiation(ssd);
    make_aged(ssd);
    //这里对请求进行预处理，保存读的时候有东西可读
    //在这一步就要分配好要读的数据的位置，写的位置在写的时候再分配
    //把读的数据放在什么地方呢？现在初步的想法是尽量先放入nvm中，多的再放到flash中，需要修改的函数是get_ppn_for_pre_process()
    ...
    while (flag != 100) {
        ...
        if (flag == 1) {
            ...
            if (ssd->parameter->dram_capacity != 0) {
                /* buffer management 这个函数主要是在dram层面对请求进行筛选分配，使dram尽可能的处理掉一部分请求
                 * 所以这个函数涉及的数据结构主要是管理buffer node的一些算法，如查找用的平衡二叉树，LRU用的队列
                 * 管理已写回的lsn的written队列
                 * TODO ：其中涉及到的create_subrequest()函数需要修改
                 */
...
*  update for nvm：
 *  这个函数对请求进行分配，尽可能使dram来完成请求，这个函数中所使用的数据结构主要涉及了:
 *  1. 用于查找buffer node的平衡搜索树
 *  2. 用于维护LRU结构的双向链表
