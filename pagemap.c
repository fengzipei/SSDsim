...
{
    unsigned int position = 0; // 这里要对请求预分配，要优先将这个物理页放在nvm上，这个指针指向当前已分配的位置最后
	unsigned int largest_nvm_position = ssd->parameter->nvmpage_num; //nvm page 范围是[0, nvmpage_num - 1]
	int fl=0;
				...
				********************************************************************************************************/
				...
				if(ssd->dram->map->map_entry[lpn].state==0)                 /*状态为0的情况*/
				{
					...
					if( position < largest_nvm_position){
						...
						ssd->nvm_head->nvmpage_head[ppn].free_state = ((~ssd->dram->map->map_entry[lpn].state) & full_page);;
						position++;
					} else {
						/**************************************************************
                        ...
                        ***************************************************************/
                        //因为nvm的存在，需要修正flash的地址，需要修改下面的两个函数
						ppn = get_ppn_for_pre_process(ssd, lsn);
						location = find_location(ssd, ppn - largest_nvm_position);
						ssd->program_count++;
...
*获取页号分为动态获取和静态获取
**************************************/
//这个函数中需要修正flash的物理地址偏差，只需在最后的位置加上一个偏移量nvmpage num
